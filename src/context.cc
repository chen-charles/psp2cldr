...
static std::shared_ptr<HandlerContinuation> _handler_call_target_function_impl(int n_params, uint32_t target_func_ptr, InterruptContext *ctx, std::string name)
{
...
std::shared_ptr<HandlerResult> InterruptContext::install_forward_handler(std::string target)
{
	...
	LOG(TRACE, "({}) fwd to {} BEGIN R0={:#010x} LR={:#010x}", this_id, target, thread[RegisterAccessProxy::Register::R0]->r(),
	...
	return handler_call_target_function(target)->then([=](uint32_t result, InterruptContext *ctx) {
		LOG(TRACE, "({}) fwd to {} END R0={:#010x}", this_id, target, ctx->thread[RegisterAccessProxy::Register::R0]->r());

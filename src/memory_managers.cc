...
MemoryScheduler::MemoryScheduler(size_t alignment, const range &memory, const std::vector<range> &occupied_ranges) : memory(memory), m_alignment(alignment)
{
	...
	std::sort(m_occupied.begin(), m_occupied.end(), [](auto &left, auto &right) { return left.first.first < right.first.first; });
}
...
uintptr_t MemoryTranslator::translate(const uintptr_t addr) const
{
	...
	auto it = std::upper_bound(m_memory_map.begin(), m_memory_map.end(), addr,
							   [](auto &left, auto &right) { return right.first.second > left; }); // first range [a, b) that has b > addr
...
uintptr_t MemoryAllocator::alloc(size_t alignment, size_t length)
{
	...
	auto ptr = (uintptr_t)VirtualAlloc2(nullptr, nullptr, length, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE, &param, 1);
	if (ptr == 0)
	{

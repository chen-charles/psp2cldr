...
uint32_t RegisterAccessProxy_NativeMSVC::w(uint32_t value)
{
	...
	*(unsigned long *)((char *)(&(m_engine->m_target_ctx)) + reg_mapping.at(name())) = value;
	return value;
}
...
int thread_handle_signal()
{
	...
	{
		LOG(TRACE, "execute({}): handling interrupt si_signo={:#x} pc={:#010x} si_addr={:#010x}", thread->tid(), thread->m_exception_record.ExceptionCode, pc,
			(uint32_t)thread->m_exception_record.ExceptionAddress);

...
static std::shared_ptr<ExecutionThread> init_main_thread(LoadContext &ctx, ExecutionCoordinator &coordinator)
{
	...
	if (!thread)
	{
		...
		coordinator.register_interrupt_callback([&ctx](ExecutionCoordinator &coord, ExecutionThread &thread, uint32_t intno) {
...
static void install_nid_stub(LoadContext &ctx, MemoryAccessProxy &proxy, uint32_t libraryNID, uint32_t functionNID, uint32_t ptr_f, bool in_place,
							 unimplemented_nid_handler stub_func)
{
...
int load_velf(const std::string &filename, LoadContext &ctx, ExecutionCoordinator &coordinator)
{
	...
	auto load_info = velf.load_and_relocate([&coordinator](uint32_t addr, size_t len) { return coordinator.mmap(addr, len); }, proxy);
	...
	for (auto &imp : imps)
	{
		...
		for (auto &ent : imp.second)
		{
			...
			if (ctx.provider() && ctx.provider()->get(libraryNID, functionNID))
			{
				...
				{
					install_nid_stub(ctx, proxy, libraryNID, functionNID, ptr_f, true, [](NID_t libraryNID, NID_t functionNID, InterruptContext *ctx) {
				...
				{
					install_nid_stub(ctx, proxy, libraryNID, functionNID, ptr_f, false, [](NID_t libraryNID, NID_t functionNID, InterruptContext *ctx) {
						return ctx->load.provider()->get(libraryNID, functionNID)(ctx);
					});
				}
			}
...
static void install_sym_stub(LoadContext &ctx, ExecutionCoordinator &coordinator, std::string sym_name, Elf32_Sym sym, uint32_t ptr_f, bool in_place,
							 unimplemented_sym_handler stub_func)
{
	auto &proxy = coordinator.proxy();
...
int load_elf(const std::string &filename, LoadContext &ctx, ExecutionCoordinator &coordinator)
{
	...
	LOG(DEBUG, "loading and applying relocations");
	auto &proxy = coordinator.proxy();
	auto load_info = elf.load_and_relocate([&coordinator](uint32_t addr, size_t len) { return coordinator.mmap(addr, len); }, proxy);
	auto &load_base = load_info.first;
	LOG(INFO, "load base={:#010x}, load top={:#010x}", load_base, load_base + load_info.second);
		...
		if (ctx.provider() && ctx.provider()->get(sym_name.c_str()))
		{
			...
			if (type_of_import == ProviderPokeResult::VARIABLE)
			{
				install_sym_stub(ctx, coordinator, sym_name, imp.first, ptr_f, true,
								 [](std::string name, Elf32_Sym sym, InterruptContext *ctx) { return ctx->load.provider()->get(name.c_str())(ctx); });
				init_routines.push_back(ptr_f);
			}
			else
			{
				...
				install_sym_stub(ctx, coordinator, sym_name, imp.first, ptr_f, false, [](std::string name, Elf32_Sym sym, InterruptContext *ctx) {
					...
					else
					{
						return std::dynamic_pointer_cast<HandlerResult>(
							ctx->handler_call_target_function(name)->then([](uint32_t result, InterruptContext *ctx) {

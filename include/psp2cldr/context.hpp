...
template <class ExceptionType,
          typename = typename std::enable_if<std::is_base_of<std::exception, ExceptionType>::value>::type>
...
class HandlerContinuation : public HandlerResult, public std::enable_shared_from_this<HandlerContinuation>
{
  public:
    ...
    virtual std::shared_ptr<HandlerContinuation> then(
        std::function<std::shared_ptr<HandlerResult>(uint32_t, InterruptContext *)> cont,
...
typedef std::function<std::shared_ptr<HandlerResult>(NID_t libraryNID, NID_t functionNID, InterruptContext *p_ctx)>
    unimplemented_nid_handler;
typedef std::function<std::shared_ptr<HandlerResult>(std::string name, Elf32_Sym sym, InterruptContext *p_ctx)>
    unimplemented_sym_handler;
...
class LoadContext
{
  ...
  public:
    ...
    std::unordered_map<uint32_t, import_stub_entry>
        unimplemented_targets; // unresolved import location, import_stub_entry

    /* VELF specific */
    std::unordered_map<NID_t, std::unordered_set<NID_t>> nids_loaded; // moduleNID, [funcNID, ...]
    std::unordered_map<NIDHASH_t, std::pair<bool, uint32_t>>
        nids_export_locations; // (moduleNID, funcNID), <isVariable, actual loaded location>
    ...
    std::unordered_map<std::string, std::pair<uint32_t, uint32_t>>
        libs_exidx; // unwind support: library_name, <exidx_la, exidx_sz>
    ...
    std::unordered_map<std::string, std::vector<uint32_t>>
        libs_preemptable_symbols; // symbol_name, [prev_import_ptr_f, ...]
    virtual std::pair<std::string, uint32_t> try_resolve_location(
        uint32_t location) const; // <library_name, offset> or library_name == ""
...
extern void panic(ExecutionCoordinator *coord, ExecutionThread *thread = nullptr, LoadContext *load = nullptr,
                  int code = 0, const char *msg = nullptr);

class InterruptContext
{
  public:
    InterruptContext(ExecutionCoordinator &coord, ExecutionThread &thread, LoadContext &load)
        : coord(coord), thread(thread), load(load)
    {
    }

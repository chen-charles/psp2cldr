...
public:
	...
	ELFLoader_Base(std::ifstream &&stream) : ifs(std::move(stream))
	{
		...
		if (std::memcmp(ehdr.e_ident, ELFMAG, SELFMAG) != 0 || ehdr.e_ident[EI_CLASS] != ELFCLASS32 || ehdr.e_ident[EI_DATA] != ELFDATA2LSB ||
			ehdr.e_ident[EI_VERSION] != EV_CURRENT || ehdr.e_machine != EM_ARM)
...
protected:
	virtual void apply_relocation(const MemoryAccessProxy &target, uint32_t code, uint32_t P, const Elf32_Sym &Sym, uint32_t A, uint32_t B_S, bool is_RelA,
								  uintptr_t load_base) const
	{
